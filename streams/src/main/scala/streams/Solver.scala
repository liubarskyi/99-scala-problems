...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])], explored: Set[Block]): Stream[(Block, List[Move])] = {
    ...
    initial #::: from(newNeighborsOnly(neighbors, explored), explored ++ initial.map({ case (block, moves) => block }))
  }
  ...
  /**
    ...
    */
  lazy val pathsToGoal: Stream[(Block, List[Move])] = pathsFromStart.filter({ case (block, moves) => block == goalBlock })
